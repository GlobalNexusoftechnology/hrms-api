...
export class PayslipService {
  ...
  async downloadPayslip(
    ...
    const companyAddress =
      [branch?.line1, branch?.line2, branch?.city, branch?.state]
        .filter(Boolean)
        .join(', ') || '123 Tech Park, Cyber City';
    ...
    doc
      .fontSize(10)
      .text(`Email: ${companyEmail} | Phone: ${companyPhone}`, 50, 75);

    doc
      .fillColor('#ffffff')
      .fontSize(22)
      .font('Helvetica-Bold')
      .text('PAYSLIP', 0, 30, { align: 'right', width: 545 });
    doc
      .fontSize(12)
      .font('Helvetica')
      .text(
    ...
    doc
      .font('Helvetica')
      .text(`${employee.firstName} ${employee.lastName}`, 150, topY + 20);

    doc.font('Helvetica-Bold').text('Designation:', 300, topY + 20);
    doc
      .font('Helvetica')
      .text(employee.designation?.name || 'N/A', 400, topY + 20);
    ...
    doc
      ...
      .text(
        formatCurrency(
          Number(payroll.baseBasicSalary) ||
    ...
    doc
      .font('Helvetica')
      .text(
        formatCurrency(Number(activeSalary?.grossSalary || 0)),
        400,
        topY + 60,
      );
    ...
    doc.text('AMOUNT', leftColX + 175, tableTop + 7, {
      width: 60,
      align: 'right',
    });
    doc.text('DEDUCTIONS', rightColX + 10, tableTop + 7);
    doc.text('AMOUNT', rightColX + 175, tableTop + 7, {
      width: 60,
      align: 'right',
    });
    ...
    const drawEarning = (label: string, amount: number, alwaysShow = false) => {
      if (amount > 0 || alwaysShow) {
        ...
        doc.text(formatCurrency(amount), leftColX + 145, earnY, {
          width: 90,
          align: 'right',
        });
        earnY += rowHeight;
    ...
    const earningComponents =
      payroll.componentsData?.filter((c) => c.type === 'EARNING') || [];
    const totalDynamicEarnings = earningComponents.reduce(
    ...
    earningComponents.forEach((c) =>
      drawEarning(c.componentName, Number(c.amount)),
    );
    ...
    if (Number(payroll.bonusAmount) > 0)
      drawEarning(
        `Bonus ${payroll.bonusReason ? `(${payroll.bonusReason})` : ''}`,
    ...
    const drawDeduction = (
      ...
      if (amount > 0 || alwaysShow) {
        ...
        doc.text(formatCurrency(amount), rightColX + 145, dedY, {
          width: 90,
          align: 'right',
        });
        dedY += rowHeight;
    ...
    const deductionComponents =
      payroll.componentsData?.filter((c) => c.type === 'DEDUCTION') || [];
    deductionComponents.forEach((c) =>
      drawDeduction(c.componentName, Number(c.amount)),
    );
    ...
    if (Number(payroll.deductionAmount) > 0)
      drawDeduction(
        `Other ${payroll.deductionReason ? `(${payroll.deductionReason})` : ''}`,
    ...
    doc.text(formatCurrency(totalEarnings), leftColX + 145, totalY + 8, {
      width: 90,
      align: 'right',
    });

    const totalDynamicDeductions = deductionComponents.reduce(
      (sum, c) => sum + Number(c.amount),
      0,
    );
    ...
    doc.text(formatCurrency(totalDeductions), rightColX + 145, totalY + 8, {
      width: 90,
      align: 'right',
    });
    ...
    doc.fontSize(16).text(formatCurrency(payroll.finalSalary), 430, netY + 14, {
      width: 105,
      align: 'right',
    });
    ...
    doc
      .fillColor(payroll.isPaid ? '#28a745' : '#dc3545')
      .font('Helvetica-Bold')
      .text(isPaidText, 135, netY + 8);

    doc.fillColor('#333333');
    const formattedDate = payroll.paidAt
      ? payroll.paidAt.toLocaleString('en-US', {
          year: 'numeric',
          month: 'short',
          day: 'numeric',
          hour: '2-digit',
    ...
    doc
      ...
      .text('Authorized Signature', 400, netY + 115, {
        width: 145,
        align: 'center',
      });
